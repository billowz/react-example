...
var builder = {
  ...
  build: function(opt) {
    ...
    build = function(basePath, dirPath, output, c) {
      output = output || 'index';
      var filePath = path.join(dirPath, output + '.jsx'), file, hasFile;
      if (!fs.existsSync(filePath)) {
        filePath = path.join(dirPath, output + '.js');
        hasFile = fs.existsSync(filePath);
      } else {
        hasFile = true;
      }
      ...
      gulp.src([path.join(dirPath, '*.js'), path.join(dirPath, '*.jsx')], '!'+filePath).on('data', function(file) {
        gutil.log('data::::'+file.path);
      }).on('end', function(){
        gutil.log('end::::'+arguments);
      });
