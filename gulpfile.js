...
gulp.task('build', ['build:styles', 'build:lib', 'build:lib:doc', 'build:dist', 'build:react', 'build:material']);
    ...
    moduleBuilder.scanFiles(path.join(__dirname, appCfg.lessThemeDir), function() {
        ...
        if (isDir) {
            ...
            gulp.src([appCfg.lessThemeDir + '/' + dirName + '/*.less',
                ...
                .pipe(sourcemaps.write())
                .pipe(gulp.dest(appCfg.distDir+'/theme'))
...
gulp.task('server', ['build:styles', 'build:module', 'build:module:doc', 'build:react', 'build:material'], function() {
    ...
    gulp.watch(appConfig.lessDir, ['build:less']);
    gulp.watch(appConfig.lessDir+'/theme.js', ['build:theme']);
    gulp.watch(appConfig.lessThemeDir+'/theme.js', ['build:theme']);
});
